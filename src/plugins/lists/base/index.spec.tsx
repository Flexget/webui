...
describe('plugins/managedList', () => {
  ...
  describe('tabs', () => {
    it('should render tab list', async () => {
      const { getByRole } = renderWithWrapper(<TestManagedList />);
      const tablist = getByRole('tablist');
      const tabs = await within(tablist).findAllByRole('tab');
      expect(tabs).toHaveLength(2);

      expect(fetchMock.called('glob:/api/managed_list/1/entries?*')).toBeTrue();
    ...
    it('should allow selecting a tab', async () => {
      ...
      expect(fetchMock.called('glob:/api/managed_list/2/entries?*')).toBeTrue();
    });
  });

  describe('EntryListHeader', () => {
    it('should open a dialog when pressing remove list button', async () => {
      const { getByLabelText, queryByRole } = renderWithWrapper(<TestManagedList />);
    ...
    it('should change order when changing the value', async () => {
      const { findAllByRole, getByLabelText } = renderWithWrapper(<TestManagedList />);
      ...
      expect(
        fetchMock.called(
          `glob:/api/managed_list/1/entries?*order=${options[1].getAttribute('data-value')}*`,
        ),
      ).toBeTrue();
    });
